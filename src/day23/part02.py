...
def is_winning_location(player: Player, location: Point, other_players: list[Player]) -> bool:
    if location.x == player.goal_room_x:
        ...
        for check_y in range(location.y + 1, 6):
            check_location = Point(location.x, check_y)
            check_player = [x for x in other_players if x.location == check_location]
            if not (len(check_player) == 1 and check_player[0].location.x == check_player[0].goal_room_x):
                return False
...
class Game:
    ...
    def possible_player_moves(self, cur_location: Point, already_visited: list[Point], player: Player, distance_moved: int) -> list[PlayerMove]:
        ...
        # if is_location_room(cur_location) and len(already_visited) > 0:
            ...
            # if this is the right room, but the wrong players are in the room
            ...
            for check_y in range(cur_location.y + 1, 6):
                check_location = Point(cur_location.x, check_y)
                check_player = [x for x in self.players if x.location == check_location]
                if len(check_player) == 1 and check_player[0].goal_room_x != cur_location.x:
                    return []
...
if __name__ == "__main__":
    start = time.time()
    result = get_part_one_result("src/day23/input_part02.txt")
    end = time.time()
